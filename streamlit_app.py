...
with col_g1:
    st.markdown('<div class="card-label">Mapa de Calor (Tamanho = Posição / Cor = Variação)</div>', unsafe_allow_html=True)
    ...
    for ticker, (qtd, pm) in CARTEIRA_US.items():
        if ticker in TOP_US or qtd * 100 > 1000: # Filtrar apenas relevantes para não poluir
            price, var = get_stock_data(ticker)
            total_reais = (price * qtd) * dolar_now
    ...
    fig_tree.update_traces(
        ...
        hovertemplate='<b>%{label}</b><br>💰 R$ %{value:,.0f}<br>📈 %{customdata[0]:.2f}%'
    )
