...
# ═══════════════════════════════════════════════════════════════
# SEÇÃO: GRÁFICOS AVANÇADOS
# ═══════════════════════════════════════════════════════════════

st.markdown('<div class="section-title"><span class="section-icon">📊</span> Raio-X da Carteira</div>', unsafe_allow_html=True)

col_g1, col_g2 = st.columns([1, 1])

# --- GRÁFICO 1: TREEMAP (MAPA DE CALOR) ---
with col_g1:
    st.markdown('<div class="card-label">Mapa de Calor (Peso vs Variação)</div>', unsafe_allow_html=True)
    ...
    for ticker, (qtd, pm) in CARTEIRA_BR.items():
        ...
        total_reais = price * qtd
        nome_curto = ticker.replace(".SA", "")
        dados_treemap.append({"Ticker": nome_curto, "Setor": "Brasil", "Valor": total_reais, "Var": var})
        
    # Processar US (Convertendo para Reais para ser comparável)
    for ticker, (qtd, pm) in CARTEIRA_US.items():
        if ticker in TOP_US or qtd * 100 > 1000: # Filtrar apenas relevantes para não poluir
            price, var = get_stock_data(ticker)
            total_reais = (price * qtd) * dolar_hj
            dados_treemap.append({"Ticker": ticker, "Setor": "EUA", "Valor": total_reais, "Var": var})

    df_treemap = pd.DataFrame(dados_treemap)
    
    # Calcular Peso Relativo (%) para o tooltip (PRIVACIDADE: esconde o valor absoluto)
    if not df_treemap.empty:
        total_carteira_grafico = df_treemap['Valor'].sum()
        df_treemap['Peso_Pct'] = (df_treemap['Valor'] / total_carteira_grafico) * 100

        # Criar Treemap
        fig_tree = px.treemap(
            df_treemap, 
            path=[px.Constant("Carteira"), 'Setor', 'Ticker'], 
            values='Valor',
            color='Var',
            color_continuous_scale=['#ff5252', '#2c2c2c', '#69f0ae'], # Vermelho -> Escuro -> Verde Neon
            color_continuous_midpoint=0,
            custom_data=['Var', 'Peso_Pct']
        )
        
        # Estilização "Glass" e Tooltip Customizado (Sem valor R$)
        fig_tree.update_traces(
            textposition="middle center",
            textfont=dict(family="Outfit", size=15),
            hovertemplate='<b>%{label}</b><br>⚖️ Peso: %{customdata[1]:.1f}%<br>📈 Var: %{customdata[0]:.2f}%<extra></extra>'
        )
        ...
        # Container com vidro para o gráfico
        st.markdown('<div class="glass-card glass-dark" style="padding: 10px;">', unsafe_allow_html=True)
        st.plotly_chart(fig_tree, use_container_width=True)
        st.markdown('</div>', unsafe_allow_html=True)

# --- GRÁFICO 2: COMPARATIVO 6 MESES (NORMALIZADO) ---
with col_g2:
    st.markdown('<div class="card-label">Batalha de Gigantes (6 Meses · Base 100)</div>', unsafe_allow_html=True)
    
    @st.cache_data(ttl=3600)
    def get_comparison_data():
        # Vamos comparar: IBOV vs S&P500 (IVV) vs Bitcoin vs Dólar
        tickers = ["^BVSP", "IVV", "BTC-USD", "USDBRL=X"]
        nomes = {"^BVSP": "Ibovespa", "IVV": "S&P 500", "BTC-USD": "Bitcoin", "USDBRL=X": "Dólar"}
        ...
        for t in tickers:
            try:
                ...
                # Normalizar: (Preço / Preço Inicial) * 100
                if len(hist) > 0:
                    hist_norm = (hist / hist.iloc[0]) * 100
                    df_temp = pd.DataFrame({
                        "Data": hist.index, 
                        "Valor": hist_norm.values, 
                        "Ativo": nomes[t]
                    })
    ...
    if not df_comp.empty:
        ...
        color_map = {
            "Ibovespa": "#00E676", # Verde Matrix
            "S&P 500": "#2979FF",  # Azul Tech
            "Bitcoin": "#FFD700",  # Dourado
            "Dólar": "#FF4081"     # Rosa/Vermelho
        }
